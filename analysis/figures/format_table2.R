...
format_table_2 <- function(table2_raw, cohort){
  table2_raw<-read.csv(paste0(results_dir,"table2_main_27022023.csv"))
  table2_raw <- table2_raw %>% select(analysis,outcome,unexposed_events, exposed_events)
  
  table2_pre_expo <- table2_raw %>% select(analysis, outcome, unexposed_events)
  table2_pre_expo$period <- "unexposed"
  table2_pre_expo <- table2_pre_expo %>% rename(event_count = unexposed_events)
  
  table2_post_expo <- table2_raw %>% select(analysis, outcome, exposed_events)
